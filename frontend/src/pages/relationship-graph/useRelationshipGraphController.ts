...
export function useRelationshipGraphController() {
  ...
  const [acceptanceStatus, setAcceptanceStatus] = useState(DISPLAY_ACCEPTANCE_STATUS);
  const [nodeType, setNodeType] = useState("");
  const [edgeType, setEdgeType] = useState("");
  const [minConfidence, setMinConfidence] = useState("");
  const [limit, setLimit] = useState("500");
  const [fromDate, setFromDate] = useState(() => localDateDaysAgo(GRAPH_MONTH_DAYS - 1));
  const [toDate, setToDate] = useState("");
  const [graphRangeDays, setGraphRangeDays] = useState(GRAPH_MONTH_DAYS);
  ...
  const anonymousGraphLabels = useMemo(
    () => buildAnonymousGraphLabels([...nodes, ...pendingNodes]),
    [nodes, pendingNodes],
  );
  ...
  const loadPendingEdges = useCallback(async () => {
    const tenantId = config.tenantId.trim();
    if (!tenantId || !selectedGroupIsVerified) {
      setPendingEdges([]);
      setPendingNodes([]);
  ...
  const showAllGraph = useCallback(async () => {
    ...
    setAcceptanceStatus(DISPLAY_ACCEPTANCE_STATUS);
    setNodeType("");
    setEdgeType("");
    setMinConfidence("");
    setFromDate("");
    ...
    await loadGraphAndStatus({
      acceptanceStatus: DISPLAY_ACCEPTANCE_STATUS,
      nodeType: "",
      edgeType: "",
      minConfidence: "",
      fromDate: "",
